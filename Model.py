...
import numpy as np
import xlearn
from Predict_Leave import DataPrepare
import tpot
import re
...
def GBDT_model(train_feature, train_labels, test_feature, test_labels):
    # GBDT  n_estimators= 120 0.71
    gbdt_model = GradientBoostingClassifier(n_estimators=120, criterion='friedman_mse')
...
def fm_model(train_feature, train_labels, test_feature, test_labels):
    # 0.73
    xlearn.write_data_to_xlearn_format(train_feature, train_labels,"../Predict_leave/1.txt")
    xlearn.write_data_to_xlearn_format(test_feature,test_labels,"../Predict_leave/2.txt")
    fm_model = xlearn.create_fm()
    fm_model.setTrain("../Predict_leave/1.txt")
    param = {'task': 'binary', 'lr': 0.2, 'lambda': 0.00002, 'metric': 'acc', 'k': 4,'opt':'sgd','epoch':50}
    fm_model.cv(param)
    fm_model.fit(param,"./model.out")
    fm_model.setSign()
    fm_model.setTest("../Predict_leave/2.txt")
...
def sum_result(*args):
    ...
    for i in range(len(seprate_result[0])):
        temp_result = 0
        for j in range(len(seprate_result) - 1):
...
def for_set_well_parameter():
    ...
    final_result = sum_result(lr_result, tpot_result, gbdt_result, gbdt_lr_result, fm_result)
